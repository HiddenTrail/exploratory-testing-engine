"""EcoEstate notes CRUD: the real target chosen after the smoke-check dress
rehearsal - self-contained, local (in-memory), and mutation-bearing, unlike
the read-only external-API-backed endpoints (property-prices, green-spaces,
walking-distance) ruled out earlier. 2 checkpoints, up to 8 tests each, real
Skeptic review each checkpoint.

Now wired to the oracle library generated by run_oracle_pass.py - 21
applicable heuristic categories, 196 vectors, 96% grounded in this project's
own real docs (acceptance criteria, risk register, known issues, tech debt).
Merged into the Driver's evidence (casting + hypothesis), same principle as
the rest of this project: it informs the Driver, never the Skeptic, which
only ever sees the Driver's own final claims.
...
MAX_CHECKPOINTS = 3
TEST_BUDGET = 8
ORACLE_LIBRARY_PATH = Path(__file__).parent / "results" / "oracle_library.json"
...
<html><head><meta charset="utf-8"><title>EcoEstate Notes CRUD PoC</title>
<style>
...
<body>
  <h1>EcoEstate Notes CRUD PoC</h1>
